...
void SwapPages(sm::Displayer &displayer, bool &is_page_1, sm::IDataPage &page1, sm::IDataPage &page2)
{
  if (is_page_1)
  {
    is_page_1 = !is_page_1;
...
void setup()
{
  ...
  scheduler.AddTask(sm::FastTask([&imu]()
                                 { imu.ReadData(); }));
  scheduler.AddTask(sm::RateTask([&gps]()
                                 { gps.ReadData(); }, 10));
  scheduler.AddTask(sm::RateTask([&displayer]()
                                 { displayer.Display(); }, 30));
  scheduler.AddTask(sm::RateTask([&displayer, &is_gps_page, &gps_data_page, &accel_data_page]()
                                 { SwapPages(displayer, is_gps_page, gps_data_page, accel_data_page); }, 0.2));
  // scheduler.AddTask(sm::RateTask([&accel_logger]()
  //                                { accel_logger.LogAccel(); }, 20));
  scheduler.AddTask(sm::RateTask([&scheduler, &usb]()
                                 { scheduler.LogLoopTime(usb); }, 10));
  scheduler.AddTask(sm::RateTask([&scheduler]()
                                 { scheduler.ClearMaxLooptime(); }, 0.5));
