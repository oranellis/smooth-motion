...
void sm::Scheduler::RunSchedule() {
  ...
  for (auto fast_task = fast_tasks_.begin(); fast_task != fast_tasks_.end();
       ++fast_task) {
    fast_task->RunTask();
  }

  for (auto rate_task = rate_tasks_.begin(); rate_task != rate_tasks_.end();
       ++rate_task) {
